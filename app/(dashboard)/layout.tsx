...
export const metadata:Metadata={title:{template:'%s | مميز',default:'مميز | إدارة المصنع'},robots:{index:false,follow:false},icons:{icon:'/LOGOMOMAYAZ.png'}};
export default async function DashboardLayout({children}:{children:React.ReactNode}){
 ...
 try{access=await getAccess();}catch{return <html lang="ar" dir="rtl"><body><main className="p-6"><p role="alert">تعذر التحقق من صلاحيات الحساب. لم تُعرض بيانات المصنع؛ أعد تحميل الصفحة.</p><Link href="/dashboard">إعادة المحاولة</Link><SignOutForm/></main></body></html>;}
 ...
 const links:NavItem[]=access?.active?[{href:'/dashboard',label:'نظرة عامة',group:'الرئيسية',icon:'grid'},...(allowed(access,'sales')||allowed(access,'production')?[{href:'/dashboard/orders',label:'أوامر التشغيل',group:'الرئيسية',icon:'factory'}]:[]),...(allowed(access,'sales')||allowed(access,'production')?[{href:'/dashboard/delivery-notes',label:'أذونات التسليم',group:'الرئيسية',icon:'file'}]:[]),...sections.filter(l=>allowed(access,l.permission)).map(l=>({...l,href:'/dashboard/'+l.href})),{href:'/dashboard/account',label:'حسابي',group:'الإدارة',icon:'settings'}]:[];
 return <html lang="ar" dir="rtl"><body className="font-sans"><DashboardShell links={links} email={user.email || ''} signOut={<SignOutForm/>}>{access?.active?children:<p role="alert" className="p-6 text-red-700">الحساب غير مفعّل للنظام. اطلب من المدير إضافة صلاحياتك أو إعادة تفعيل الحساب.</p>}</DashboardShell></body></html>;
}

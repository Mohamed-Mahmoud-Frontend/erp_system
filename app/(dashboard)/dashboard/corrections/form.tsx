...
export default function CorrectionForm(props:Props) {
  ...
  const [attStatus, setAttStatus] = useState(props.kind === "attendance" ? props.status : "present");
  return <form action={action} className="space-y-3 border rounded p-3" data-correction={props.kind}>
    ...
    {props.kind === "attendance" && (
      <div className="space-y-3">
        <label className="block text-sm font-semibold">الحالة<select className="border rounded p-2 w-full mt-1" name="status" value={attStatus} onChange={e => setAttStatus(e.target.value)}><option value="present">حاضر</option><option value="half_day">نصف يوم</option><option value="quarter_day">ربع يوم</option><option value="absent">غائب</option></select></label>
        {attStatus !== "absent" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-slate-50 rounded border">
            <label className="block text-xs font-medium">نوع الإضافة<select className="border rounded p-2 w-full mt-1 bg-white" name="extra_type" defaultValue={props.extraType}><option value="amount">مبلغ بالجنيه</option><option value="day_fraction">جزء من اليومية الحالية</option></select></label>
            <label className="block text-xs font-medium">قيمة الإضافة<input className="border rounded p-2 w-full mt-1 bg-white" name="extra_units" type="number" min="0" step="any" defaultValue={props.extraUnits} required/></label>
          </div>
        ) : (
          <>
            <input type="hidden" name="extra_type" value="amount" />
            <input type="hidden" name="extra_units" value="0" />
            <p className="text-xs text-amber-700 bg-amber-50 p-2 rounded">تم تحديد العامل كغائب (لا تحسب أي إضافات).</p>
          </>
        )}
      </div>
    )}

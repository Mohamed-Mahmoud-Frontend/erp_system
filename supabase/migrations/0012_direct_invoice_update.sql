-- Migration: 0012_direct_invoice_update.sql
-- Update direct invoice RPC to accept an existing client ID OR create a new client

DROP FUNCTION IF EXISTS create_direct_invoice_atomic(text, text, text, integer, numeric, numeric);

CREATE OR REPLACE FUNCTION create_direct_invoice_atomic(
  p_client_id uuid,          -- NULL if new client
  p_client_name text,        -- NULL if existing client
  p_client_type text,        -- NULL if existing client
  p_client_phone text,       -- NULL if existing client
  p_quantity int,
  p_total numeric,

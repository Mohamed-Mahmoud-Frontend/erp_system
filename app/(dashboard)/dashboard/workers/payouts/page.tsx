...
import { payrollSchema, payrollWeek, cairoToday } from "@/lib/payroll";
...
export default async function PayoutsPage({ searchParams }: { searchParams: Promise<{ week?: string; search?: string }> }) {
  ...
  const pending = rows.filter(r => !r.paid_at && new Decimal(r.net_amount).gte(0)).reduce((sum,r) => sum.plus(r.net_amount),new Decimal(0));
  const debt = rows.filter(r => !r.paid_at && new Decimal(r.net_amount).lt(0)).reduce((sum,r) => sum.plus(new Decimal(r.net_amount).abs()),new Decimal(0));
  const canPayAll=rows.length>0&&rows.every(r=>!r.paid_at&&new Decimal(r.net_amount).gte(0));
  return <div className="space-y-6 max-w-screen-xl mx-auto">
    ...
    <section className="grid gap-4 lg:grid-cols-2"><AttendanceForm workers={rows.map(r=>({id:r.worker_id,name:r.worker_name}))} today={cairoToday()}/><div className="rounded-xl border bg-white p-6"><h2 className="font-bold">كل اليومية من مكان واحد</h2><p className="mt-2 text-sm">اختر العامل واليوم، وسجل الحضور والإضافي والسلفة والمكافأة والخصم مرة واحدة. راجع الرصيد ثم اصرف من الجدول أدناه.</p></div></section>
    <section className="border rounded-xl bg-white p-6 space-y-3"><h2 className="text-xl font-bold">مجموع الصافي غير المصروف: {money(pending)} ج.م</h2>
      ...
      {debt.gt(0)&&<p role="alert" className="font-bold text-red-700">إجمالي المديونيات على العمال: {money(debt)} ج.م</p>}
      {canPayAll?<PayForm workers={rows.map(r => r.worker_id)} weekStart={week.start} all />:<p className="text-amber-800">الصرف الجماعي غير متاح عند وجود رصيد سالب أو عامل سبق صرفه.</p>}
    </section>
    ...
    <div className="overflow-x-auto bg-white border rounded-xl"><table className="w-full text-right text-sm">
      ...
      <tbody>{rows.map(row => <tr key={row.worker_id} data-worker-id={row.worker_id} className="border-t">
        ...
        <td data-field="net_amount" className={`p-3 font-bold ${new Decimal(row.net_amount).isNegative() ? "text-red-700" : "text-blue-700"}`}>{money(row.net_amount)}{new Decimal(row.net_amount).isNegative()&&<span className="block text-xs" role="alert">العامل عليه فلوس؛ لا يوجد رصيد كافٍ للصرف</span>}</td>
        <td className="p-3">{row.paid_at ? <span className="text-green-700">مصروف — {new Date(row.paid_at).toLocaleString("ar-EG",{timeZone:"Africa/Cairo"})}</span> : new Decimal(row.net_amount).isNegative()?<span className="text-red-700">مديونية — لا يمكن الصرف</span>:<PayForm workers={[row.worker_id]} weekStart={week.start} />}</td>
      </tr>)}</tbody>
